...
- `eda_etl.ipynb` should derive `steamrec_app_category_mapping.{parquet,csv}` by sorting the original category ids in increasing order, assigning dense `app_category_id` values from `1..n`, adding the Stage 4 category description, and writing columns `app_category_id`, `app_category`, `category_description`, and `count`.
