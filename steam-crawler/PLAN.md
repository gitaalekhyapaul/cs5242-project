...
- Stage 4a:
  load `stage_04_selected_games.csv`, preserve Stage 4's sampled `appid` and `recommendations_total`, then fetch `appdetails` one app at a time using `filters=basic,price_overview,genres` and fetch one reviews page per app with `num_per_page=1`. Convert `price_overview.final` from cents to currency units, fall back to `price_overview.initial` if `final` is absent, set free games to `0.0`, compute `%positive_reviews = total_positive / total_reviews * 100` from `query_summary`, store Steam genre ids in `app_category`, and append rows incrementally to `stage_04a_selected_games.csv`. Write `stage_04a_genre_mapping.csv` with the genre id to genre name lookup for the downstream mapping table. Missing values should be serialized as empty CSV cells rather than the literal string `<NA>`. Write the parquet copy only after the CSV patch step has finished.
- Stage 5a:
...
- `eda_etl.ipynb` should then derive `steamrec_app_metadata.{parquet,csv}` from the Stage 4a output by scaling `%positive_reviews` into `0-1`, filling missing `price` values with `0.0`, and keeping `app_category` as arrays of Steam genre ids.
- `eda_etl.ipynb` should derive `steamrec_app_category_mapping.{parquet,csv}` by sorting the Steam genre ids in increasing order, assigning dense `app_category_id` values from `1..n`, adding the Stage 4a genre description, and writing columns `app_category_id`, `app_category`, `category_description`, and `count`.
...
- `eda_etl.ipynb` should also expose a Stage 5a user-diagnostics section that counts reviews per `user_id`, uses `USER_REVIEW_COUNT_THRESHOLD` as a minimum user-history filter with `review_count >= USER_REVIEW_COUNT_THRESHOLD` for downstream SteamRec ETL, plots the review-count histogram on logarithmic axes with `0-1`, `1-2`, `2-3`, `3-4`, and `4-5` buckets before widening bucket sizes after `5`, and assigns chronological `position` values within each eligible user history before any later ETL is chosen.
- In that position output, `app_id` should be remapped to the dense SteamRec `item_id` while keeping the column name `app_id`, and `app_category` should carry the mapped SteamRec genre-id array for the corresponding app.
- `eda_etl.ipynb` should then derive `steamrec_interactions.{parquet,csv}` from `user_review_positions_df` by renaming `app_id` to `item_id`, renaming `review_score` to `rating`, renaming `review_rating` to `review_upvotes`, and preserving the mapped `app_category` arrays on each interaction row.
- `eda_etl.ipynb` should then derive `steamrec_sequences.{parquet,csv}` from `steamrec_interactions` with one row per `user_id`, where `test_sequence` is the full mapped item history ordered by `position`, `train_sequence` is all but the last two items, `validation_sequence` is all but the last item, `validation_target` is the second-last item, `test_target` is the last item, and the aligned metadata arrays are `timestamps`, `ratings`, `review_upvotes`, `app_category`, `app_num_reviews`, `app_avg_rating`, and `app_price`.
- `app_num_reviews`, `app_avg_rating`, and `app_price` in `steamrec_sequences` should be sourced from `steamrec_app_metadata` through the original `app_id -> item_id` mapping in `steamrec_item_mapping`, while all final sequence columns keep the mapped SteamRec ids.
...
- `eda_etl.ipynb` should end with one shared Kaggle upload cell and a Kaggle sanity-check cell that work against whichever raw and ETL parquet resources are available locally, including `steamrec_interactions.parquet`, `steamrec_sequences.parquet`, and the Kaggle aliases `final_app_category.parquet`, `final_item_mapping.parquet`, and `final_sequences.parquet`; `final_item_mapping.parquet` should expose `app_package` instead of `app_id`, and the final verification cell should retry Kaggle downloads with exponential backoff from a 10-second base for up to 10 retries, capped at 300 seconds per wait.
