...
- `eda_etl.ipynb` should end with one shared Kaggle upload cell and a Kaggle sanity-check cell that work against whichever raw and ETL parquet resources are available locally, including `steamrec_interactions.parquet` and `steamrec_sequences.parquet`.
